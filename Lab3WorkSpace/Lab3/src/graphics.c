...
#include "controllers/bunker.h"		//For bunkers
...
#define TANK_Y 	200					//The Y coordinate for the tank

//Offsets
#define TANK_RIGHT_OFFSET 7			//Offset for half tank width, used to draw/erase
...
void drawAlienBlock(bool guise_in)
{
	...
	sprite_data_t prev_sprite;	//Pointer to previous sprite
	int16_t prevX, prevY, currX, currY;	//Position variables
	...
	for(row = 0; row < ALIENS_ROWS; row++) //Loop through alien rows
	{
		...
		{
			//Choose correct guise for current sprite
			sprite = guise_in ? sprite_getAlienTopIn() : sprite_getAlienTopOut();
			//Choose correct guise for previous sprite
			prev_sprite = guise_in ? sprite_getAlienTopOut() : sprite_getAlienTopIn();
		}
		else if(row < ALIENS_BOTTOM_ROW)
		{
			...
			//Choose correct guise for previous sprite
			prev_sprite = guise_in ? sprite_getAlienMiddleOut() : sprite_getAlienMiddleIn();
		}
		else
		{
			//Choose correct guise for current sprite
			...
			//Choose correct guise for previous sprite
			prev_sprite = guise_in ? sprite_getAlienBottomOut() : sprite_getAlienBottomIn();
		}
		for(col = 0; col < ALIENS_COLS; col++) //Loop through alien columns
		{
			if(aliens_getStatus(row*ALIENS_COLS + col))	//Check if alien is alive
			{
				//xil_printf("Drawing alien\r\n");
				//Erase old alien
				graphics_drawSprite(prev_sprite, prevX + col*ALIEN_CELL_WIDTH, prevY + row*ALIEN_CELL_HEIGHT, BLACK);
				//Draw new alien
...
{
	uint16_t bunker;	//Looping variable
	sprite_data_t bunker_sprite = sprite_getBunker();	//Sprite of bunker
	for(bunker = 0; bunker < BUNKER_NUM_BUNKERS; bunker++)	//Loop through bunkers
	{
		//Draw the bunker
		graphics_drawSprite(bunker_sprite, bunker_getX(bunker), BUNKER_Y, GREEN);
	}
}

/*
 * Draw the full tank
 * @param	erase	if set true it will erase the tank
 */
void graphics_drawFullTank(bool erase)
{
	//get the sprite for the full tank
	sprite_data_t tankSpriteData = sprite_getTank();
	//get the tank's current location
	int16_t curr_tank_pos = tank_getPosition();
	//set the color based on erase
	uint32_t color = erase ? BLACK : GREEN;
	//draw the tank to the screen
	graphics_drawSprite(tankSpriteData, curr_tank_pos, TANK_Y, color);
}
...
void graphics_updateTank()
{
	...
	int16_t prev_tank_pos = tank_getPreviousPosition();
	//get the tank's current location
	int16_t curr_tank_pos = tank_getPosition();
	...
	int16_t leftPos, rightPos;


	//check the direction of the tank
	if(prev_tank_pos < curr_tank_pos)//moving right
	{
		colorLeft = BLACK;	//erase color
		colorRight = GREEN;	//draw color
		leftPos = prev_tank_pos;	//set left position to prev tank position
		rightPos = curr_tank_pos;	//set right position to curr tank position
	}
	else if(prev_tank_pos > curr_tank_pos)//moving left
	{
		colorLeft = GREEN;	//draw color
		colorRight = BLACK;	//erase color
		leftPos = curr_tank_pos;	//set left position to curr tank position
		rightPos = prev_tank_pos;	//set right position to prev tank position
	}
	else//hasn't moved
	{
		return;
	...
	//get the sprite data
	sprite_data_t tankSpriteData = sprite_getTankLeft();

	//RIGHT
	graphics_drawSpriteReverse(tankSpriteData, rightPos + TANK_RIGHT_OFFSET, TANK_Y, colorRight);

	//LEFT
	graphics_drawSprite(tankSpriteData, leftPos, TANK_Y, colorLeft);
...
/*
 * Draws a sprite to the screen with columns reversed
 * @param	sprite		sprite data
 * @param	x			x-coordinate (320x240 coordinates)
 * @param	y			y-ccordinate (320x240 coordinates)
 * @param	color		Color to draw sprite
...
{
	int16_t row, col;		//Looping variables for unpacking sprite data
	uint32_t coord640;	//Index of starting coordinate (x,y) in 640x480 coordinates
	coord640 = y*PIXEL_BLOWUP*SCREEN_WIDTH + x*PIXEL_BLOWUP;	//Convert 320x240 position coordinates to 640x480 starting index
	for(row = 0; row < sprite.height; row++)	//Loop over rows in sprite
	{
		for(col = sprite.width - 1; col >= 0; col--)	//Loop over columns in sprite
		{
			if(sprite.sprite_data[row] & (0x1 << col))	//Check if current pixel is supposed to be colored in
			{
				//Color in 4 640x480 pixels for each 320x240 pixel
